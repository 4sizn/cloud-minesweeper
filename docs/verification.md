...
- COCO-Stuff 주석(CC BY 4.0)과 상업 이용 가능한 COCO 사진에서 하늘 크롭 490장 생성 후 학습.
- 별도 평가 132장: 셀 IoU **63.5% → 74.2%**, 최종 구름 조각 IoU **56.6% → 67.2%**(이전 색상 규칙 대비).
- PyTorch와 ONNX 출력 일치를 Python에서 확인했다.
- 새 픽스처 3장(`cloud_cases.dart`)을 iPhone 17 Pro 시뮬레이터의 iOS ONNX 런타임으로 추론.
  Python 평가와 최종 선택 셀 집합이 정확히 일치했고, 사진 통합 테스트 4건이 모두 통과했다.
  실기기 재확인은 하지 않았다.
